...
OptBWLNS_cpp::OptBWLNS_cpp( DAG* _dag , ADDAG *_addag ):OptBW( _dag , _addag , NULL ){}

void OptBWLNS_cpp::run_adanalysis(ostream &os)
{
...
void OptBWLNS_cpp::gen_func_body( ostream &os ){
...
double OptBWLNS_cpp::get_emax( Edge *CEdge ){
...
void OptBWLNS_cpp::gen_func_top( ostream &os ){
  ...
  for( int i = 0; i < ad_dag->ad_ndx_map.size() ; i++ ){
    ...
    if (  dynamic_cast< HWlns *>(CEdge->my_var)->frac_bw_state != SET )
      {
	...
	var_init << "pow( 2. , " << edge_exp << " + pow( 2. , -x[ " << ctr  << " ] - 1 ) ) - ";

	var_init << "pow( 2. , " << edge_exp << " + pow( 2. , -x[ " << ctr  << " ]  ) );\n";
...
void OptBWLNS_cpp::gen_error_func( ostream &os ){
...
void  OptBWLNS_cpp::update_edges(double *cost_params){
  ...
  for ( int ctr = 0; ctr < bs->param_name_map.size() ; ctr++ ){
    ...
    if ( temp = dynamic_cast<HWlns *>( CEdge->my_var ) ){
      ...
      temp->int_bw =  static_cast<fsize>(bs->range_analysis->range_bw_lns(CEdge));
...
void OptBWLNS_cpp::update_edges_uni(double uni_bw ){
