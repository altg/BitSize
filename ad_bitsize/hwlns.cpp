...
void HWlns::init(arch_attr Signal_type, fsize tsize, fsize msize, sign_attr sign, string name)
{

  if ( ( bs->mydag->arith_type == AT_NOTSET ) || ( bs->mydag->arith_type == LNS ) )
    bs->mydag->arith_type = LNS;
  else
  ...
  // _DBOUT(  "HWlns const called " << var_name )
  ...
  if ( Signal_type == IN )
    {
      ...
      Node* temp_node = bs->mydag->new_node( INPUT , LNS ); 
  ...
  else if ( Signal_type == OUT )
    {
      ...
      Node* temp_node_n = bs->mydag->new_node( OUTPUT , LNS );
  ...
  if ( (tsize == OPT_BW ) && ( msize == OPT_BW ) )
    ...
    {
      int_bw_state = NOTSET; int_bw = 0;
      frac_bw_state = SET; frac_bw = msize;

      //cout << var_name << " : " << int_bw << endl;

    }
  else if( (tsize != OPT_BW ) && ( msize == OPT_BW ) )
    {
      frac_bw_state = NOTSET; frac_bw = 0;
      int_bw_state = SET; int_bw = tsize - OPT_BW;
...
// 	  { int_bw_state = SET; int_bw = tsize - OPT_BW;  }
    ...
    {
      int_bw_state = SET; int_bw = tsize - msize;
      frac_bw_state = SET; frac_bw = msize;
    }
  else
    {
...
HWlns::HWlns(arch_attr Signal_type, fsize tsize, R_ERR _r_err, sign_attr sign, string name)
{
  ...
  double int_bw2 = static_cast<double>( int_bw );
  
  double bias = pow( 2. , int_bw2 - 1 ) - 1;

  double maxE = pow( 2. , int_bw2 ) -2 - bias;
...
HWlns::HWlns(arch_attr Signal_type, fsize tsize, fsize msize, sign_attr sign, string name)
{
  ...
  //double maxE = pow( 2. , int_bw2 ) -2 - bias;
  //double maxE = pow( 2. , int_bw2 ) - bias;

  //cout << int_bw2 << " ::\n";

  myedge->e_min = -pow( 2. , pow( 2. , int_bw2 - 1) );
  myedge->e_max = pow( 2. , pow( 2. , int_bw2 - 1) );

  //cout << myedge->e_min << " :: " <<  myedge->e_max << endl;
...
HWlns::HWlns ( const HWlns &a ):HWvar(a){
	
  //	ad_val = a.ad_val;

}

double HWlns::get_val() {

	return static_cast<double>(ad_val);

 }
 
HWlns& HWlns::operator= ( const HWlns &r){
...
HWlns& HWlns::operator= (double val) {
...
// 	  ad_val.fix_int_state = NOTSET;
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Node* const_node = bs->mydag->new_const_node( LNS , val  );
...
void HWlns::analyse_bw( int bw , int mode )
{
...
HWlns operator+ (const HWlns &l,const HWlns &r)  { 
	...
	temp.ad_val = l.ad_val + r.ad_val;
	 ...
	 if ( bs->global_opmode == PARSING )
	  {
	    Node* op_node = bs->mydag->add_op_node( ADD , LNS , l.myedge , r.myedge  );
	 ...
	 if ( bs->global_opmode == PARSING )
	  {
	    ...
	    Node* op_node = bs->mydag->add_op_node( ADD ,  LNS ,const_edge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator+  (const HWlns &l, const double dval)  { 
	...
	temp.ad_val = l.ad_val + dval;
	 ...
	 if ( bs->global_opmode == PARSING )
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( ADD , LNS , l.myedge , const_edge );
	    temp.myedge =  bs->mydag->new_edge(  NULL , "TEMP"  );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator-  (const HWlns &l, const HWlns &r)  { 
	...
	temp.ad_val = l.ad_val - r.ad_val;
		...
		if ( bs->global_opmode == PARSING )
	   {	    
	     Node* op_node = bs->mydag->add_op_node( SUB , LNS , l.myedge , r.myedge  );
	     temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	     bs->mydag->add_out_edge( op_node , temp.myedge );
	...
	if ( bs->global_opmode == PARSING )
	  {
	    ...
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( SUB , LNS , const_edge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator-  (const HWlns &l, const double dval)  { 
	...
	temp.ad_val = l.ad_val - dval;
	  ...
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( SUB , LNS , l.myedge , const_edge );
	    temp.myedge =  bs->mydag->new_edge(  NULL , "TEMP"  );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator*  (const HWlns &l, const HWlns &r)  {
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Node* op_node = bs->mydag->add_op_node( MUL , LNS , l.myedge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator*  ( const double dval, const HWlns &r)  { 
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( MUL , LNS , const_edge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator*  (const HWlns &l, const double dval)  { 
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( MUL , LNS , l.myedge , const_edge );
	    temp.myedge =  bs->mydag->new_edge(  NULL , "TEMP"  );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator/  (const HWlns &l, const HWlns &r)  {
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Node* op_node = bs->mydag->add_op_node( DIV , LNS , l.myedge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator/  ( const double dval, const HWlns &r)  { 
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( DIV , LNS , const_edge , r.myedge  );
	    temp.myedge =  bs->mydag->new_edge( NULL , "TEMP" );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
HWlns operator/  (const HWlns &l, const double dval)  { 
	...
	if ( bs->global_opmode == PARSING )
	  {
	    Edge* const_edge = bs->mydag->new_const_edge( dval );
	    Node* op_node = bs->mydag->add_op_node( DIV , LNS , l.myedge , const_edge );
	    temp.myedge =  bs->mydag->new_edge(  NULL , "TEMP"  );
	    bs->mydag->add_out_edge( op_node , temp.myedge );
...
void HWlns::read_inputdata( double dbval )
{
...
void HWlns::output_analysis( int mode )
{
