...
CostFunction::CostFunction( DAG *_dag ):BitSizePass(_dag){}

void CostFunction::cost_function_end(ostream &os ){
...
//   os << "C = tot_A * tot_B;\n";
  os << "C = ( range_bw_A + prec_bw_A ) * (  range_bw_B +  prec_bw_B );\n";
...
string CostFunction::state_bw(  Edge *CEdge )
{
...
double CostFunction::range_bw( Edge *CEdge )
{
...
void CostFunction::gen_parameters_func( ostream &os ){
...
void CostFunction::gen_matlab_costf( ostream& os ,  string err_func_name )
{
  ...
  os << "function [cost_value, cost_flag]  = cost_func_area( x )" << endl;
  os << "global bw_out;" << endl;

  os << "[no_of_bw, bw_out_ , bw_names, bw_type,  bw_set , bw_ranges ] = parameters();" << endl;
...
void CostFunction::gen_ml_node_cf( Node *CNode )
{
...
void CostFunction::gen_ml_add_node_exp(  Node * CNode )
{
...
void CostFunction::gen_ml_sub_node_exp(  Node * CNode )
{
...
void CostFunction::gen_ml_mul_node_exp(  Node * CNode )
{
  ...
  if ( src_l_node->node_type == CONSTANT ){
    ...
    f_body << node_costnames[ CNode ] << " = cost_cmul( ";
    ...
    f_body << node_costnames[ CNode ] << " = cost_mul( " << l_range_bw  << " , " << l_edge  << " , ";
    f_body <<  r_range_bw << " , " << r_edge << " , " << out_range_bw  << " , " << out_edge  << " );" << endl;
  }
...
void  CostFunction::gen_ml_div_node_exp(  Node * CNode )
{
  ...
  f_body << node_costnames[ CNode ] << " = cost_div( " << l_range_bw  << " , " << l_edge  << " , ";
  f_body <<  r_range_bw << " , " << r_edge << " , " << out_range_bw  << " , " << out_edge  << " );" << endl;
