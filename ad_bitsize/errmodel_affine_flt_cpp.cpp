...
ErrModel_Affine_FLT_CPP::ErrModel_Affine_FLT_CPP(DAG *_dag , NodeSchedular *_node_sched  ):BitSizePass( _dag ),node_sched(_node_sched),affine_index(1),affine_error_analysis_done(false){}


void ErrModel_Affine_FLT_CPP::do_affine_analysis()
{
...
void ErrModel_Affine_FLT_CPP::do_affine_analysis_node(dag_index node_num)
{
...
string ErrModel_Affine_FLT_CPP::get_abs_eterms( stringstream &ss , Edge *CEdge  , stringstream &eterm)
{
...
void ErrModel_Affine_FLT_CPP::affine_const_node_exp( Node *CNode )
{
...
void ErrModel_Affine_FLT_CPP::affine_assign_node( Node *CNode )
{
...
void ErrModel_Affine_FLT_CPP::affine_add_node( Node *CNode )
{
  ...
  //if(( node_l->node_type != CONSTANT )&& ( node_r->node_type != CONSTANT )){
	...
	if  ( ctr < edge_l->af_err_vec.size() )
	  ...
	  if( l_val != "" ){
	    ...
	    if( dynamic_cast<HWfloat *>(edge_l->my_var)->man_bw_state == NOTSET )
	      {
		temp_l << l_val;
	      }
	    else
	  ...
	  if( r_val != "" ){
	     ...
	     if( dynamic_cast<HWfloat *>(edge_r->my_var)->man_bw_state == NOTSET )
	       {
		 temp_r << r_val;
	       }
	      else
...
void ErrModel_Affine_FLT_CPP::affine_sub_node( Node *CNode )
{
  Edge *edge_l = c_dag->edge_vec[ CNode->in_edges[0] ];
  Edge *edge_r = c_dag->edge_vec[ CNode->in_edges[1] ];
  Edge *edge_out = c_dag->edge_vec[ CNode->out_edges.back() ];
...
void  ErrModel_Affine_FLT_CPP::affine_div_node( Node *CNode ){
  Edge *edge_l = c_dag->edge_vec[ CNode->in_edges[0] ];
  Edge *edge_r = c_dag->edge_vec[ CNode->in_edges[1] ];
  Edge *edge_out = c_dag->edge_vec[ CNode->out_edges.back() ];
...
double ErrModel_Affine_FLT_CPP::get_abs_maxval( Edge *CEdge ){
...
void ErrModel_Affine_FLT_CPP::affine_mul_node( Node *CNode )
{
...
void ErrModel_Affine_FLT_CPP::affine_shiftl_node( Node *CNode )
{
  Edge *edge_l = c_dag->edge_vec[ CNode->in_edges[0] ];
  Edge *edge_r = c_dag->edge_vec[ CNode->in_edges[1] ];
  Edge *edge_out = c_dag->edge_vec[ CNode->out_edges.back() ];
...
void ErrModel_Affine_FLT_CPP::affine_shiftr_node( Node *CNode )
{
  Edge *edge_l = c_dag->edge_vec[ CNode->in_edges[0] ];
  Edge *edge_r = c_dag->edge_vec[ CNode->in_edges[1] ];
  Edge *edge_out = c_dag->edge_vec[ CNode->out_edges.back() ];
...
void ErrModel_Affine_FLT_CPP::gen_signal_bwname( stringstream &ss, Edge* CEdge)
{
...
void ErrModel_Affine_FLT_CPP::gen_signal_errorname( stringstream &ss, Edge* CEdge)
{
...
void ErrModel_Affine_FLT_CPP::gen_signal_err_expr( stringstream &ss, Edge *CEdge )
{
...
void ErrModel_Affine_FLT_CPP::affine_input_node( Node *CNode )
{
...
void ErrModel_Affine_FLT_CPP::show_affine_vecs()
{
...
double  ErrModel_Affine_FLT_CPP::get_emax( Edge *CEdge ){
...
void ErrModel_Affine_FLT_CPP::gen_matlab_err_func( ostream &os)
{
